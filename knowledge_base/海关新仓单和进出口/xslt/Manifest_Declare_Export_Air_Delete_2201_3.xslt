	...
	<xsl:template match="/">
		<Manifest>
			<xsl:for-each select="ns0:Manifest">
				...
				<xsl:variable name="var4_JourneyID" select="$var1_Declaration/ns0:BorderTransportMeans/ns0:JourneyID"/>
				<xsl:variable name="var5_resultof_equal" select="($var3_resultof_cast = 'MT1101')"/>
				<xsl:variable name="var6_resultof_substring" select="substring($var3_resultof_cast, number('4'), number('1'))"/>
				<xsl:variable name="var7_resultof_cast" select="string($var2_Head/ns0:SenderID)"/>
				<xsl:variable name="var8_resultof_cast" select="string($var2_Head/ns0:ReceiverID)"/>
				<xsl:variable name="var9_resultof_cast" select="string($var4_JourneyID)"/>
				<xsl:variable name="var10_val">
					<xsl:choose>
				...
				<content>
					...
					<bills>
						<xsl:for-each select="$var1_Declaration/ns0:Consignment">
							<xsl:variable name="var11_AssociatedTransportDocument" select="ns0:AssociatedTransportDocument"/>
							<xsl:variable name="var12_resultof_exists" select="boolean($var11_AssociatedTransportDocument)"/>
							<xsl:variable name="var13_TransportContractDocument" select="ns0:TransportContractDocument"/>
							<bill>
								<manifest_id>
									<xsl:choose>
										<xsl:when test="string((string($var10_val) = '2')) != 'false'">
								...
								<bill_no>
									<xsl:choose>
										<xsl:when test="string((string($var12_resultof_exists) != 'false')) != 'false'">
											<xsl:for-each select="$var11_AssociatedTransportDocument">
												<xsl:value-of select="string(ns0:ID)"/>
											</xsl:for-each>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="string($var13_TransportContractDocument/ns0:ID)"/>
										</xsl:otherwise>
									</xsl:choose>
								</bill_no>
								<bill_type>
								...
								<xsl:if test="string((string($var12_resultof_exists) != 'false')) != 'false'">
									<parent_bill_no>
										<xsl:value-of select="string($var13_TransportContractDocument/ns0:ID)"/>
									</parent_bill_no>
								</xsl:if>
								...
								<xsl:for-each select="$var13_TransportContractDocument/ns0:ChangeReasonCode">
									<xsl:variable name="var14_resultof_concat" select="concat(string(.), ';')"/>
									<xsl:variable name="var15_resultof_substring" select="substring($var14_resultof_concat, '1', (string-length($var14_resultof_concat) - '1'))"/>
									<xsl:if test="string(not((string(string-length($var15_resultof_substring)) = '0'))) != 'false'">
										<change_reason>
											<xsl:value-of select="$var15_resultof_substring"/>
										</change_reason>
									</xsl:if>
								</xsl:for-each>
							</bill>
